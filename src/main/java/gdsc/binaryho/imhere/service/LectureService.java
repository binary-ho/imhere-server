...
public class LectureService {
    ...
    public void createLecture(Long lecturerId, String className) {
        Member lecturer = memberRepository.findById(lecturerId).orElseThrow();
        if (!lecturer.hasRole(Role.LECTURER)) {
            /* TODO: Exception 만들어서 대체 */
            throw new IllegalArgumentException();
        }
        Lecture newLecture = Lecture.createLecture(lecturer, className);
